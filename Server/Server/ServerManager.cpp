...
UINT __cdecl ServerManager::DataThreadFunc(LPVOID pParam)
{
    ...
    const char* message = "Welcome to the chat room.\n";
    ...
    while ((recv_size = recv(pYourSocket, server_reply, 2000, 0)) != SOCKET_ERROR)
    {
        ...
        std::string formatted_message = std::string(server_reply) + "\n";

        // Broadcast the message to all connected clients
        for (int i = 1; i <= iCount; i++)
        {
            if (send(sArray[i], formatted_message.c_str(), formatted_message.length(), 0) < 0)
            {
                puts("Send failed");
            }
        }
